...
///source: Halim CP3 4.2.7 Graph Edges Property Check via DFS Spanning Tree
...
int v, e;
vi parent, vis; // 0 -> unvisited, 1 -> visited, 2 -> explored
vii crossEdge, backEdge;
...
int main() {
    ...
    if (siz(backEdge)) {
        cout << "Cyclic" << endl; // important use
        for (auto &x : backEdge)
            cout << x.ff << " " << x.ss << endl;
    }
    if (siz(crossEdge)) { // for directed graph
        cout << "some cross edges:" << endl;
