...
/* source : Competitive Programmer's Handbook: 6.4 Minimizing Sums
 *Given an array of n elements; if c == 1 : find such x so that |a1 - x| + |a2 - x| + ... + |an - x| is minimum
 *answer is found for such x that is the median of the array values
 * if c == 2 : find such x so that (a1 - x) ^ 2 + ... + (an - x) ^ 2 is minimum
 * x is the rounded average of the values for integer x
 */

ll minSums(vi &ara, int c) {
    ll mini = 0, x = 0;
    if (c == 1) {
        sort(all(ara));
        x = ara[siz(ara) / 2];
...
int main() {
    ...
    cout << minSums(ara, 1) << endl;
    cout << minSums(ara, 2) << endl;
