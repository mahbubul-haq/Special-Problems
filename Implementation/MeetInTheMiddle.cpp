...
///source : Competitive Programmer's Handbook, 5.5 Meet In the Middle

void getsums(int i, int n, int sum, int id, vvi &sums, vi *ara, bool taken) {
    if (i == n) {
        if (taken)
            sums[id].pb(sum);
        return;
    ...
    getsums(i + 1, n, sum + ara[id][i], id, sums, ara, true);
    getsums(i + 1, n, sum, id, sums, ara, taken);
}
...
int main() {
    ...
    for (int i = 0; i < n; i++)  {
        if (i & 1)
            ara1[0].pb(ara[i]);
        else ara1[1].pb(ara[i]);
    }

    vvi sums(2);

    getsums(0, siz(ara1[0]), 0, 0, sums, ara1, false);
    getsums(0, siz(ara1[1]), 0, 1, sums, ara1, false);
