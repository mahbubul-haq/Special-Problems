...
///Source: Competitive Programmer's Handbook 8.2 Nearest Smaller Element

vi nearestSmaller(vi &ara) {
    int n = siz(ara);
    vi ans(n);
    stack<int> stk;
    for (int i = 0; i < n; i++) {
        while (!stk.empty() and stk.top() >= ara[i])
            stk.pop();
        if (stk.empty())
            ans[i] = -1;
        else ans[i] = stk.top();
