...
void minNumofCoins(vi &minCoins, vi &coins) {
    int n = siz(coins), make = siz(minCoins) - 1;
    minCoins[0] = 0;
    for (int &x : coins) {
        for (int i = 1; i <= make; i++) {
            if (i - x >= 0 and minCoins[i - x] != INT_MAX) {
                minCoins[i] = min(minCoins[i], 1 + minCoins[i - x]);
            }
        }
    }
}
...
int main() {
    ...
    int n, make;
    cin >> n >> make; /// n = number of coins, make = amount of money to change

    vi coins(n); /// values of the coins

    for (int &x : coins) {
        cin >> x;
    }

    vi minCoins(make + 1, INT_MAX); /// minimum number of coins to make the values

    minNumofCoins(minCoins, coins);

    cout << minCoins[make] << endl;
