...
/*
 * calculates the maximum sum we can get if we
 * go only down and right through the n * m grid from
 * (0, 0) to (n - 1, m - 1)
 *Sample Test:
 *  5 5
    3 7 9 2 7
...
ll maxSum(vvi &mat) {
    int n = siz(mat), m = siz(mat[0]);
    ll sum[n + 1][m + 1];
    mem(sum, 0);

    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            sum[i + 1][j + 1] = max(sum[i][j + 1], sum[i + 1][j]) + mat[i][j];
        }
    }
