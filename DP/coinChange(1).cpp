...
int main() {
    ...
    ///minimum number of coins needed to make the value: make
    /// each coin has infinite supply
    ...
    int first[make + 1];
    vi minCoins(make + 1, INT_MAX);
    minCoins[0] = 0;

    for (int i = 1; i <= make; i++) {
        for (int &c : coins) {
            if (i - c >= 0 and minCoins[i - c] != INT_MAX) {
    ...
    ///number of ways to make the value: make
    ///order of the coins matters

    int numofWays[make + 1];
    mem(numofWays, 0);
    numofWays[0] = 1;
    ...
    for (int i = 1; i <= make; i++) {
        for (int &c : coins) {
            if (i - c >= 0) {
                numofWays[i] += numofWays[i - c];
            }
        }
    }

    cout << numofWays[make] << endl;
